...
export default function ProjectDetail() {
  ...
  if (!project) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <button onClick={() => navigate('/')} className="text-amber-400/70 hover:text-amber-300 text-sm transition">
          返回项目列表
        </button>
      </div>
    );
  ...
  return (
    <div className="flex h-[calc(100vh-52px)]">
      {/* 左侧导航栏 */}
      <aside className="w-52 glass border-r border-border-subtle flex flex-col shrink-0 ink-wash-bg">
        {/* 返回按钮 */}
        <button
          onClick={() => navigate('/')}
          className="flex items-center gap-2 px-4 py-3 text-sm text-gray-400 hover:text-gray-200 hover:bg-white/[0.03] transition border-b border-border-subtle"
        >
        ...
        <div className="px-4 py-3 border-b border-border-subtle">
          <h2 className="font-semibold text-sm truncate text-gray-200">{project.name}</h2>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 text-sm rounded-lg transition ${
                  activeTab === tab.key
                    ? 'bg-white/[0.06] text-amber-300/80 shadow-[inset_2px_0_0] shadow-amber-500/60'
        ...
        <div className="px-4 py-3 border-t border-border-subtle">
          <div className="text-xs text-gray-500 mb-1.5">写作进度</div>
          <div className="w-full bg-white/[0.04] rounded-full h-1.5 overflow-hidden">
            <div
              className="h-1.5 rounded-full transition-all duration-500 shadow-sm shadow-amber-500/20"
              style={{
          ...
          <div className="flex gap-1.5">
            {[
              ...
              <button
                ...
                className="flex-1 text-xs py-1.5 bg-white/[0.03] hover:bg-white/[0.06] text-gray-400 hover:text-gray-200 rounded-md border border-border-subtle hover:border-border-hover transition"
              >
                {label}
              </button>
            ))}
